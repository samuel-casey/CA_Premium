let x;
let changeDirection;
const startSim = document.querySelector('#startSim');
const Y_AXIS = 1;
const X_AXIS = 2;
let b1, b2, c1, c2;
...
const allStationCoordinates = [{'lat':'30.279','lon':'-87.555'},
...
const nextTide = document.querySelector('#nextTide');
const previousTide = document.querySelector('#lastTide');
...
function handleSubmit() {
    ...
    $.get('https://maps.googleapis.com/maps/api/geocode/json', locationParams, function (response, textStatus, jqXHR) {
        ...
        for (index = 0; index < allStationCoordinates.length; index++) {
            var milesFromInputtedLocation = distance(googleLat, googleLon, allStationCoordinates[index].lat, allStationCoordinates[index].lon); // should output 1000 something miles if NY = input
        ...
        // set the coords equal to that of the closest station
        coordinates = {
            'lat': closest_station.lat,
            'lon': closest_station.lon
        };

        $.post('/station', coordinates, function stationName(response) {stationOutput.append(response['station_name']);} );

        $.post('/times', coordinates, (response) => {if (currentTime.innerHTML.length < 2) { currentTime.append("Local time: \n" + response.slice(11,16)); startSim.append('Simulate to next tide') }});
        
        $.post('/nextTide', coordinates, (response) => {if (nextTide.innerHTML.length < 2) { nextTide.append("Next tide: \n" + response.slice(11,16))} } );
        
        $.post('/lastTide', coordinates, (response) => {if (lastTide.innerHTML.length < 2) { lastTide.append("Last tide: \n" + response.slice(11,16))} } );
    
        $.post('/beachSize', coordinates, (response) => {console.log(response);} )
    });
