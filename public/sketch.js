...
function addStationEl(station) {
    const stationNameEl = `<div>${station["station_name"]}</div>`
...
function handleSubmit() {
    ...
    .then((response) => {
        ...
        for (index = 0; index < allStationCoordinates.length; index++) {
            var milesFromInputtedLocation = distance(googleLat, googleLon, allStationCoordinates[index].lat, allStationCoordinates[index].lon);
        ...
        return new Promise ( (resolve, reject ) => {
            if (coordinates) {
                ...
                reject("Couldn't find coordinates for inputted location")
            }
        })
    })
    ...
    // $.post('/times', coordinates, (response) => {
    //     const timeEl = `<div><b>Local time: ${response.slice(11,16)}</b></div>`
...
//     $.post('/nextTide', coordinates, (response) => {
//         const nextTideObj = response.rows[0]
//         const lastTideObj = response.rows[1]

//         const nextDateTime = nextTideObj["date_time"].trim().slice(11,16)
...
//         //last tide
//         const lastDateTime = lastTideObj["date_time"].trim().slice(11,16)
